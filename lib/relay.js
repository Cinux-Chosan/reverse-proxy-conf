 ...
 * -----
 * Last Modified: Saturday, 18th May 2019 7:43:08 pm
...
const updateReq = (req) => {
  ...
  return new Promise((res, rej) => {
    const delayReq = request({ url }).on('response', (response) => {
      const resCache = { data: Buffer.alloc(0), interval };
      response
        .on('data', data => {
...
module.exports = (() => {
  ...
  return async (req, res) => {
    ...
    if (interval) {
      // 设置了间隔时间，需要做请求缓存
      reqCache[toUrl] = reqCache[toUrl] || { resList: [], reqLock: false, resData: {} }
...
setInterval(() => {
  for (const url in reqCache) {
    if (reqCache.hasOwnProperty(url)) {
      const cacheData = reqCache[url];
      if (cacheData) {
        const { resData: { lastTime, interval } } = cacheData;
